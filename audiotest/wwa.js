...
(function (wwa_parts_player) {
    ...
    var Player = (function (_super) {
        ...
        Player.prototype.isControllable = function () {
            return (this._state === 0 /* CONTROLLABLE */ && !this._partsAppeared && ((this._wwa.getMapTypeByPosition(this._position) !== Consts.MAP_LOCALGATE && this._wwa.getMapTypeByPosition(this._position) !== Consts.MAP_URLGATE) || !this._wwa.isPrevFrameEventExecuted()) && this._moveMacroWaitingRemainMoves === 0 && this._moveObjectAutoExecTimer === 0);
        };
        ...
        Player.prototype.addItem = function (objID, itemPos, isOverwrite) {
            ...
            if (itemPos === 0) {
                ...
                if (this._wwa.getObjectAttributeById(oldObjID, Consts.ATR_NUMBER) !== this._wwa.getObjectAttributeById(objID, Consts.ATR_NUMBER)) {
                    insertPos = this._getBlankItemPos();
                    if (insertPos !== Consts.ITEMBOX_IS_FULL) {
                        this._itemBox[insertPos - 1] = oldObjID;
                    }
                }
            }
            else {
        ...
        Player.prototype.readyToUseItem = function (itemPos) {
            ...
            //            this._wwa.enqueueMessage(this._wwa.getMessageById(messageID), itemID, wwa_data.PartsType.OBJECT, this._position.getPartsCoord());
            this._wwa.setMessageQueue(this._wwa.getMessageById(messageID), false, itemID, 0 /* OBJECT */, this._position.getPartsCoord());
        ...
        Player.prototype.resetMoveObjectAutoExecTimer = function () {
            this._moveObjectAutoExecTimer = Consts.CHIP_SIZE / Consts.DEFAULT_SPEED;
        ...
        Player.prototype.decrementMoveObjectAutoExecTimer = function () {
            if (this._moveMacroWaitingRemainMoves >= 0 && this._moveObjectAutoExecTimer > 0) {
                return --this._moveObjectAutoExecTimer;
            }
            return 0;
        };
...
(function (wwa_message) {
    var MessageInfo = (function () {
        function MessageInfo(message, isEndOfPartsEvent, macro) {
    ...
    function strArrayToMessageInfoArray(strArray) {
        var newq = [];
        strArray.forEach(function (s) {
            newq.push(new MessageInfo(s));
        });
        return newq;
    }
    ...
    var WWA = (function () {
        ...
        function WWA(mapFileName, workerFileName, urlgateEnabled) {
            ...
            loadWorker.addEventListener("message", function (e) {
                ...
                _this._lastMessage = new wwa_message.MessageInfo("", false, []);
                ...
                _this._cgManager = new CGManager(ctx, ctxSub, _this._wwaData.mapCGName, function () {
                    util.$id("wwa-cover").addEventListener("touchstart", function () {
                        _this.loadSound();
                        _this._playSound = function (s) {
                            _this._audiojsElements[s].play();
                        };
                        setTimeout(_this.soundCheckCaller, Consts.DEFAULT_FRAME_INTERVAL, _this);
                    });
                    util.$id("wwa-cover").addEventListener("mousedown", function () {
                        _this.loadSound();
                        _this._playSound = function (s) {
                            _this._audiojsElements[s].play();
                        };
        ...
        WWA.prototype.loadSound = function () {
            var _this = this;
            // ここから試験中
            this._audioElements = new Array(Consts.SOUND_MAX + 1);
            this._audiojsElements = new Array(Consts.SOUND_MAX + 1);
            this._audioState = new Array(Consts.SOUND_MAX + 1);
            this._audioElements[1] = document.createElement("audio");
            this._audioElements[1].setAttribute("preload", "auto");
            this._audioElements[1].src = "./audio/" + 1 + ".mp3";
            util.$id("wwa-wrapper").appendChild(this._audioElements[1]);
            this._audiojsElements[1] = audiojs.create(this._audioElements[1]);
            this._audioElements[3] = document.createElement("audio");
            this._audioElements[3].setAttribute("preload", "auto");
            this._audioElements[3].src = "./audio/" + 3 + ".mp3";
            util.$id("wwa-wrapper").appendChild(this._audioElements[3]);
            this._audiojsElements[3] = audiojs.create(this._audioElements[3]);
            for (var pid = 1; pid < this._wwaData.mapPartsMax; pid++) {
                var idx = this._wwaData.mapAttribute[pid][Consts.ATR_SOUND];
                if (idx === 0 || this._audioElements[idx] !== void 0) {
                    continue;
                }
                (function (idx) {
                    _this._audioElements[idx] = document.createElement("audio");
                    _this._audioElements[idx].setAttribute("preload", "auto");
                    _this._audioElements[idx].src = "./audio/" + idx + ".mp3";
                    util.$id("wwa-wrapper").appendChild(_this._audioElements[idx]);
                    _this._audiojsElements[idx] = audiojs.create(_this._audioElements[idx]);
                    /*
                    this._audiojsElements[idx].addListener("canplaythrough", ():void => {
                        this._audioState[idx]=true;
                    } );
                    */
                })(idx);
            }
        ...
        WWA.prototype.checkAllSoundLoaded = function () {
            for (var i = 1; i <= Consts.SOUND_MAX; i++) {
                if (this._audioElements[i] === void 0) {
                    continue;
                }
                if (this._audioElements[i].error !== null) {
                    continue;
                }
                if (this._audiojsElements[i].wrapper.classList.contains("loading")) {
                    // 読み込まれていない音がある時は20ms後にリトライ
        ...
        WWA.prototype.openGameWindow = function () {
            ...
            this._messageWindow = new wwa_message.MessageWindow(this, 50, 180, 340, 75, "", this._wwaData.mapCGName, false, true, util.$id("wwa-wrapper"));
            this._monsterWindow = new wwa_message.MosterWindow(this, new Coord(50, 180), 340, 60, false, util.$id("wwa-wrapper"), this._wwaData.mapCGName);
            this._scoreWindow = new wwa_message.ScoreWindow(this, new Coord(50, 50), false, util.$id("wwa-wrapper"));
            setTimeout(function () {
        ...
        */
        ...
        WWA.prototype._main = function () {
            ...
            if (this._player.getPosition().isJustPosition() && this._camera.getPosition().isScreenTopPosition()) {
                if (!this._player.isJumped() && !this._player.isWaitingMessage() && !this._player.isWaitingMoveMacro()) {
                ...
                if (this._player.isWaitingMessage() && this._messageWindow.isYesNoChoice() && this._yesNoJudge !== 2 /* UNSELECTED */ && !this._player.isWaitingMoveMacro()) {
        ...
        WWA.prototype._drawPlayer = function (cpParts, cpOffset, yLimit, isPrevCamera) {
            ...
            if ((dir === 0 /* LEFT */ || dir === 1 /* RIGHT */) && Math.abs(poso.x) > Math.floor(Consts.CHIP_SIZE / 2) || (dir === 3 /* UP */ || dir === 2 /* DOWN */) && Math.abs(poso.y) > Math.floor(Consts.CHIP_SIZE / 2)) {
                dcrop = 1;
            }
            if (isPrevCamera) {
                this._cgManager.drawCanvasWithLowerYLimit(this._wwaData.playerImgPosX + relpcrop + dcrop, this._wwaData.playerImgPosY, canvasX, canvasY, yLimit);
            }
            else {
                this._cgManager.drawCanvasWithUpperYLimit(this._wwaData.playerImgPosX + relpcrop + dcrop, this._wwaData.playerImgPosY, canvasX, canvasY, yLimit);
            }
        };
        // 物体描画
        ...
        WWA.prototype._execMapStreetEvent = function (pos, partsID, mapAttr) {
            ...
            this.setMessageQueue(message, false, partsID, 1 /* MAP */, pos.clone());
            return messageID !== 0;
        };
        ...
        WWA.prototype._execObjectMessageEvent = function (pos, partsID, objAttr) {
            ...
            this.setMessageQueue(message, false, partsID, 0 /* OBJECT */, pos);
            // 待ち時間
            ...
            // alpha
            var id = this._wwaData.objectAttribute[partsID][Consts.ATR_SOUND];
            if (id !== 0) {
                this._audiojsElements[id].play();
            }
        };
        WWA.prototype._execObjectStatusEvent = function (pos, partsID, objAttr) {
            ...
            this.setMessageQueue(message, false, partsID, 0 /* OBJECT */, pos.clone());
            this._wwaData.mapObject[pos.y][pos.x] = 0;
            this.appearParts(pos, 1 /* OBJECT */, partsID);
        };
        ...
        WWA.prototype._execObjectBuyEvent = function (pos, partsID, objAttr) {
            ...
            // 試験的に(ry
            this.setMessageQueue(message, true, partsID, 0 /* OBJECT */, pos.clone());
            this._yesNoChoicePartsCoord = pos;
            this._yesNoChoicePartsID = partsID;
                        ...
                        if (partsType === Consts.OBJECT_BUY) {
                            if (this._player.hasItem(this._wwaData.objectAttribute[this._yesNoChoicePartsID][Consts.ATR_ITEM])) {
                                ...
                                if (this._wwaData.message[7 /* NO_ITEM */] !== "BLANK") {
                                    this._messageQueue.push(new wwa_message.MessageInfo(this._wwaData.message[7 /* NO_ITEM */] === "" ? "アイテムをもっていない。" : this._wwaData.message[7 /* NO_ITEM */]));
                                }
                                ;
                            }
                        }
                            ...
                            if (this._player.hasGold(this._wwaData.objectAttribute[this._yesNoChoicePartsID][Consts.ATR_GOLD])) {
                                if (this._player.canHaveMoreItems()) {
                                    ...
                                    // アイテムをボックスがいっぱい
                                    this._messageQueue.push(new wwa_message.MessageInfo("これ以上、アイテムを持てません。"));
                                }
                            }
                            else {
                                // 所持金が足りない
                                if (this._wwaData.message[6 /* NO_MONEY */] !== "BLANK") {
                                    this._messageQueue.push(new wwa_message.MessageInfo(this._wwaData.message[6 /* NO_MONEY */] === "" ? "所持金が足りない。" : this._wwaData.message[6 /* NO_MONEY */]));
                                }
                            }
                        }
        ...
        WWA.prototype.setMessageQueue = function (message, showChoice, partsID, partsType, partsPosition) {
        ...
        /*
            ...
            */
        WWA.prototype.getMessageQueueByRawMessage = function (message, partsID, partsType, partsPosition) {
            // コメント削除
            var messageMain = message.split(/\<c\>/i)[0].replace(/\<p\>\n/ig, "<P>");
            var messageQueue = [];
            if (messageMain !== "") {
        ...
        WWA.prototype._setNextMessage = function () {
